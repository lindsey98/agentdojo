...
from agents.camel.capabilities import Capabilities
from agents.camel.interpreter import value
from agents.camel.utils.quarantined_llm import NotEnoughInformationError
...
def _needs_json_conversion(obj: object) -> bool:
    """Check if obj contains any BaseModel or datetime (recursively)."""
    if isinstance(obj, pydantic.BaseModel):
        return True
    elif isinstance(obj, (datetime.datetime, datetime.date, datetime.time, datetime.timedelta)):
        return True
    elif isinstance(obj, dict):
        return any(_needs_json_conversion(v) for v in obj.values())
    elif isinstance(obj, (list, tuple)):
        return any(_needs_json_conversion(item) for item in obj)
    return False
...
def camel_repr(obj: object) -> str:
    """Custom repr that returns JSON for BaseModel, dict, datetime, and nested structures."""
    if _needs_json_conversion(obj) or isinstance(obj, dict):
        return json.dumps(_to_json_serializable(obj))
    return repr(obj)


def camel_str(obj: object) -> str:
    """Custom str that returns JSON for BaseModel, dict, datetime, and nested structures."""
    if _needs_json_conversion(obj) or isinstance(obj, dict):
        return json.dumps(_to_json_serializable(obj))
    return str(obj)
...
BUILT_IN_FUNCTIONS: dict[str, value.CaMeLBuiltin] = {
    ...
    "repr": value.make_camel_builtin("repr", camel_repr),
    ...
    "str": value.make_camel_builtin("str", camel_str),
...
BUILT_IN_CLASSES: dict[str, value.CaMeLClass] = {
    ...
    "TzInfo": value.CaMeLClass(
        "TzInfo",
        TzInfo,
        Capabilities.camel(),
        (),
        {
            "utcoffset": value.make_camel_builtin("utcoffset", TzInfo.utcoffset),
            "tzname": value.make_camel_builtin("tzname", TzInfo.tzname),
            "dst": value.make_camel_builtin("dst", TzInfo.dst),
        },
        is_totally_ordered=False,
        is_builtin=True,
    ),
