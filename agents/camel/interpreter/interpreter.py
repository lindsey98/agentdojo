...
from agents.camel.capabilities.readers import Public
from agents.camel.capabilities.sources import Tool
from agents.camel.interpreter import namespace as ns
from agents.camel.interpreter import (
    op_protocols,
    result,
    value,
)
from agents.camel.interpreter.library import _needs_json_conversion, _to_json_serializable
...
def _eval_formatted_value(
    ...
    # Convert BaseModel, datetime, dict with datetime to JSON for consistent formatting
    if _needs_json_conversion(str_val) or isinstance(str_val, dict):
        str_val = json.dumps(_to_json_serializable(str_val))
    try:
