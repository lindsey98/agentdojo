...
from agents.camel.utils import quarantined_llm
from agents.camel.utils import system_prompt_generator
from agents.camel.capabilities import is_trusted
from agents.camel.interpreter import interpreter, result
from agents.camel.interpreter import namespace as ns
from agents.camel.interpreter.value import CaMeLValue
from agents.camel.pipeline_elements.agentdojo_function import (
    make_agentdojo_namespace,
)
from agents.camel.security_policies import (
    AgentDojoSecurityPolicyEngine,
)
from agents.camel.utils.security_policy import SecurityPolicyDeniedError
...
def safe_to_json_str(obj: object) -> str:
    """Convert object to JSON string, handling datetime and BaseModel."""
    return json.dumps(_to_json_serializable(obj))
...
def extract_print_output(tool_calls: Sequence[interpreter.FunctionCall]) -> str:
    ...
    for print_call in print_calls:
        for arg_value in print_call.args.values():
            printed_output += safe_to_json_str(arg_value)
    return printed_output


def function_call_from_ad_function_call(function_call: interpreter.FunctionCall) -> functions_runtime.FunctionCall:
    args = {k: (repr(v) if isinstance(v, type) else _to_json_serializable(v)) for k, v in function_call.args.items()}
...
def format_camel_exception(camel_exception: interpreter.CaMeLException, code: str) -> str:
    ...
    # Handle case where nodes is empty (e.g., SecurityPolicyDeniedError)
    if not camel_exception.nodes:
        if is_trusted(camel_exception):
            exception_text = str(exception)
        else:
            print(f"\n[DEBUG redacted_exception] Actual exception: {type(exception).__name__}: {exception}\n", flush=True)
...
class PrivilegedLLM(agent_pipeline.BasePipelineElement):
    ...
    def run_code(
        ...
        try:
            ...
            # Handle security policy denial gracefully - return error info without crashing
            print(f"\n[DEBUG] SecurityPolicyDeniedError: {e}\n", flush=True)
            error = interpreter.CaMeLException(
                e,
                (),  # empty nodes tuple
                ()   # empty values tuple
            )
            return ("", [], error, namespace, dependencies)

        printed_output = extract_print_output(tool_calls)
        if printed_output:
            print(f"\n[DEBUG print_output] Extracted print output:\n{printed_output}\n")
    ...
    def query(
        ...
        # Lazy imports to avoid circular import with agentdojo.default_suites
        ...
        classes_to_exclude = (
            ...
            if not isinstance(env, WorkspaceEnvironment | TravelEnvironment | CamelBypassEnvironment)
            else set()
        )
        ...
        if not isinstance(env, WorkspaceEnvironment | CamelBypassEnvironment):
        ...
        if messages[-1]["role"] == "user":
            user_content = ad_types.get_text_content_as_str(messages[-1]["content"])
            # Handle both regular tracebacks and SecurityPolicyDeniedError (which has no traceback)
            if "Running the code gave the following error:" in user_content:
                messages = [*messages, ad_types.ChatAssistantMessage(role="assistant", content=None, tool_calls=None)]

        # Add system message to messages for logging
        system_message = ad_types.ChatSystemMessage(
            role="system",
