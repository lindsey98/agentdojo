...
from agentdojo.default_suites.v1.camel_bypass_poc import tools as camel_bypass_tools

from agents.camel.capabilities import Capabilities, readers, sources
...
def _get_camel_bypass_email_metadata(email: camel_bypass_tools.Email, tool: str) -> Capabilities:
    email_readers = frozenset({email.sender} | set(email.recipients))
    return Capabilities(frozenset({sources.Tool(tool, _transform_user_email_to_user({email.sender}))}), email_readers)
...
def _get_metadata_for_ad(
    ...
    match result.raw:
        ...
        case camel_bypass_tools.Email():
            return result.new_with_metadata(_get_camel_bypass_email_metadata(result.raw, tool))
