...
def query_quarantined_llm(
    ...
    """
    print(f"\n[DEBUG quarantined_llm] ENTERED function with query length={len(query)}, schema={output_schema}\n", flush=True)
    ...
    print(f"\n[DEBUG quarantined_llm] Query input:\n{query}\n", flush=True)

    try:
        res = model.run_sync(query).output
        print(f"[DEBUG quarantined_llm] Result: {res}\n", flush=True)
    except Exception as e:
        print(f"[DEBUG quarantined_llm] Exception: {type(e).__name__}: {e}\n", flush=True)
        raise
    ...
    if not res.have_enough_information:  # type: ignore
        print(f"[DEBUG quarantined_llm] NotEnoughInformationError - model said not enough info\n", flush=True)
