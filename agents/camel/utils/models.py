...
from agents.camel.pipeline_elements.replay_privileged_llm import PrivilegedLLMReplayer, UserInjectionTasksGetter
from agents.camel.security_policies import (
    ...
    CamelBypassSecurityPolicyEngine,
...
_SECURITY_POLICY_ENGINES: dict[str, type[AgentDojoSecurityPolicyEngine]] = {
    ...
    "camel_bypass_poc": CamelBypassSecurityPolicyEngine,
}
...
def make_tools_pipeline(
    ...
    use_security_policies: bool = False,
    ...
    if use_original:
        ...
        if _is_oai_reasoning_model(model):
            pipeline_name = f"camel_{model.split(':')[1]}-{reasoning_effort}"
        else:
            pipeline_name = f"camel_{model.split(':')[1]}"
        tools_pipeline = agent_pipeline.AgentPipeline(
            [
                agent_pipeline.InitQuery(),
        ...
        if "openai" in model and _is_oai_reasoning_model(model):
            tools_pipeline.name = f"camel_secpol_{model.split(':')[1]}-{reasoning_effort}"
        elif "anthropic" in model and (("3-7-sonnet" in model or "sonnet-4" in model) and thinking_budget_tokens):
            tools_pipeline.name = f"camel_secpol_{model.split(':')[1]}-{thinking_budget_tokens}"
        else:
            tools_pipeline.name = f"camel_secpol_{model.split(':')[1]}"
        if eval_mode == MetadataEvalMode.STRICT:
            tools_pipeline.name = tools_pipeline.name.replace("camel_secpol_", "camel_secpol_strict_")
    else:
        # Use suite-specific policy engine if use_security_policies is True
        security_policy_engine = engine if use_security_policies else ADNoSecurityPolicyEngine
        ...
        # Used for logging
        if "openai" in model and _is_oai_reasoning_model(model):
            tools_pipeline.name = f"camel_{model.split(':')[1]}-{reasoning_effort}"
        elif "anthropic" in model and (("3-7-sonnet" in model or "sonnet-4" in model) and thinking_budget_tokens):
            tools_pipeline.name = f"camel_{model.split(':')[1]}-{thinking_budget_tokens}"
        else:
            tools_pipeline.name = f"camel_{model.split(':')[1]}"

        if q_llm:
            tools_pipeline.name += f"-q:{q_llm.split(':')[1]}"

        # Add suffix for security policies
        if use_security_policies:
            if eval_mode == MetadataEvalMode.STRICT:
                tools_pipeline.name += "+secpol+strict"
            else:
                tools_pipeline.name += "+secpol"
