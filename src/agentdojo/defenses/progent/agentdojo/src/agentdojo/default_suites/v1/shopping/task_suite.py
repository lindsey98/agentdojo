...
class ShoppingEnvironment(TaskEnvironment):
...
def input_to_webpage(shopping_platform: Annotated[ShoppingPlatform, Depends("shopping_platform")], email_database: Annotated[EmailDatabase, Depends("email_database")], bank_database: Annotated[BankDatabase, Depends("bank_database")], filesystem: Annotated[Filesystem, Depends("filesystem")], web_database: Annotated[WebDatabase, Depends("web_database")], url: str, input_ids: List[str], input_values: List[str]):
...
# task_suite = TaskSuite[ShoppingEnvironment]("shopping", ShoppingEnvironment, [make_function(tool) for tool in TOOLS])
...
if suite != "shopping":
    ...
    task_suite = TaskSuite[ShoppingEnvironment]("shopping", ShoppingEnvironment, [make_function(apply_secure_tool_wrapper(tool)) for tool in TOOLS])
    update_always_allowed_tools(["get_balance", "get_shopping_account_user_information", "search_product", "view_order_history", "get_invoice", "view_cart", "cart_total", "view_available_discounts", "get_shopping_linked_unread_emails", "get_shopping_linked_received_emails", "list_directory", "read_file", "get_current_day", "get_day_calendar_events"])