...
def _openai_to_tool_call(tool_call: ChatCompletionMessageToolCall) -> FunctionCall:
    args = _parse_tool_call_arguments(tool_call.function.arguments, tool_call.function.name)
    return FunctionCall(
...
def _parse_tool_call_arguments(raw_arguments: object, function_name: str) -> dict:
    ...
    print(
        f"[Agent] Warning: unable to parse tool arguments for function '{function_name}'. Fallback to empty args. Raw: {raw_arguments}",
        file=sys.stderr,
    )
    return {}
